...
- **RPC API** (§12.6, in `supabase/migrations/0002_rpc.sql` + `0003`-`0007`): student `load_project`/`save_project`/`project_history`/`project_version`; teacher `teacher_classes`/`create_class`/`add_students_bulk`/`append_student`/`teacher_roster`/`reprint_codes`/`set_readonly`/`mark_complete` (all class-scoped: append/roster/reprint take a `class_id`; the single-arg compat shims were dropped in `0006`); owner (service_role) `mint_teacher`/`export_student`/`delete_student`. `teacher_roster` returns each student's latest meaningful `line_count` (`0005`) and `readonly` flag (`0007`). A teacher can lock a project read-only (`set_readonly`, `0007`): `save_project` then rejects writes (`error:'readonly'`) and the student page goes view-only. Call directly from the browser via PostgREST `POST /rest/v1/rpc/<fn>` with the publishable key; codes are passed as args. `save_project` is durable only on `{ok:true}` and returns a `conflict` flag.
