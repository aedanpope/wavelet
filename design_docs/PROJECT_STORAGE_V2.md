...
- **RPC API** (§12.6, in `supabase/migrations/0002_rpc.sql` + `0003`-`0006`): student `load_project`/`save_project`/`project_history`/`project_version`; teacher `teacher_classes`/`create_class`/`add_students_bulk`/`append_student`/`teacher_roster`/`reprint_codes`/`mark_complete` (all class-scoped: append/roster/reprint take a `class_id`; the single-arg compat shims were dropped in `0006`); owner (service_role) `mint_teacher`/`export_student`/`delete_student`. `teacher_roster` returns each student's latest meaningful `line_count` (`0005`). Call directly from the browser via PostgREST `POST /rest/v1/rpc/<fn>` with the publishable key; codes are passed as args. `save_project` is durable only on `{ok:true}` and returns a `conflict` flag.
