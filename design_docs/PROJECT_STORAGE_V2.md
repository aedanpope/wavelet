...
> Status: **V1 complete (flag-gated, not yet switched on for the cohort).** Architecture decided (§12), DB ops workflow defined (§13). The whole server-mode flow is built and live behind `WaveletConfig.serverStorage` (default off) + the per-visit `?storage=server` override: word-based code login with "is this you?" + an explicit confirmation modal, hard-source-of-truth autosave with version history, this-cohort file import (modal chain), and the multi-class teacher dashboard (owner-minted teachers, class list, live 30s roster with net "lines you wrote", reveal/copy codes, code-table + large "strips" + progress-pack PDFs). Migrations `0001`-`0006` are applied to the live project. What's left is post-V1: the **start** cover sheet, **going live** (flip the flag), and P2/P3 ops (`pg_cron`, dev/prod split + CI, login rate-limiting) plus the Cloudflare/DNS cutover. See §14.
...
- ✅ **Step 7: Teacher entity + multi-class** (merged, PRs #48-#55): a `teachers` table owns many classes, one teacher code unlocks all of them; owner-minted teachers self-serve classes. DB layer merged (PR #48): `0003_teachers.sql` (teacher_classes / create_class / add_students_bulk (target count + a candidate code pool, spares cover collisions) / owner mint_teacher; class-scoped roster/reprint/append with single-arg compat shims), validated 22/22 against the live DB. Dashboard UI now class-aware: class-list view, create-class, per-class roster, optional-name add, bulk blank-name codes, and a compact Name+Code print table for classroom handout. The client wrappers use the class-scoped RPCs; the single-arg compat shims (and the `earliest_class` helper) were dropped in `0006_drop_compat_shims.sql`. Owner tool `admin.html` generates a teacher code + the matching `mint_teacher(...)` SQL to paste into the Supabase SQL editor (no DB access from the page). The roster shows each student's net "lines you wrote": `0005_roster_line_count.sql` adds each student's latest meaningful line count to `teacher_roster`, and the dashboard subtracts the project's starter baseline computed locally via the shared pure module `project-source.js` (same starter-assembly + meaningful-line definition as the student History view, kept in sync with `project.js`). The Project column is dropped (the project name moves into the class heading).
- ✅ **Step 8: Student + teacher UX polish** (merged, PRs #49-#57): teacher dashboard hides the code after login behind a **Log out** (code field is `type=password`); student gets a "you're logged in" modal + a `🔑 <code>` nav chip; file import is a modal chain (replace? → Saving… → "Imported and saved", only after the save confirms); roster shows net "lines you wrote" (`0005`) and **auto-refreshes every 30s** (silent, no flicker); code table downloads as a normal PDF and a large "strips" PDF for cutting up; `admin.html` mints teacher codes; compat shims dropped (`0006`).

### Remaining (post-V1)
- ⏭️ **Start cover sheet** (the one V1 feature gap, §4A): assessed-prose front matter + a milestone checklist. The final/progress-pack sheet is done; `cover-sheet.js` + the dashboard wiring are the template to copy.
- ⏭️ **Go live:** flip `WaveletConfig.serverStorage` on for the cohort (today it's default off; reachable per-visit via `?storage=server`). Decide rollout (one class first).
- 🅿️ **P2: `pg_cron` jobs** for 12-month name auto-deletion (§5) and post-completion snapshot compaction (§6).
- 🅿️ **P2: dev/prod Supabase split + CI** (§13.7). One project is "prod for now"; migrations are applied by hand.
- 🅿️ **P3: brute-force login rate-limiting** (per-code, not per-IP, §11).
- 🅿️ **Infra: Cloudflare Pages + DNS cutover.** Move prod off GitHub Pages onto Cloudflare (unifies preview/prod, unlocks `_headers` + edge functions, fixes the `version.js` merge-conflict churn). Blocked on recovering the GoDaddy login (DNS is GoDaddy, registrar likely GoDaddy AU; move nameservers to Cloudflare).
- 🔁 **Refactor (non-feature, paused): split `project.js`** (PR #44). The ~2.4k-line file works; the split is cleanup, not a blocker.
...
- **RPC API** (§12.6, in `supabase/migrations/0002_rpc.sql` + `0003`-`0009`): student `load_project`/`save_project`/`project_history`/`project_version`; teacher `teacher_classes`/`create_class`/`rename_class`/`add_students_bulk`/`append_student`/`teacher_roster`/`reprint_codes`/`set_readonly`/`set_assigned`/`mark_complete` (all class-scoped: append/roster/reprint take a `class_id`; the single-arg compat shims were dropped in `0006`); owner (service_role) `mint_teacher`/`export_student`/`delete_student`. `teacher_roster` returns each student's latest meaningful `line_count` (`0005`), `readonly` (`0007`), and `assigned` (`0008`) flags. A teacher can lock a project read-only (`set_readonly`, `0007`): `save_project` then rejects writes (`error:'readonly'`) and the student page goes view-only (the dashboard "Open ↗" link inspects a project the same way, via `?inspect=1`). `set_assigned` (`0008`) marks codes given to real students so the progress pack prints a (blank) page for kids who did not submit while skipping unused spares; `rename_class` (`0009`) renames a class. Progress-pack / code-table names can be typed per row on the dashboard and are kept in the teacher's browser (localStorage), never sent to the server. Call directly from the browser via PostgREST `POST /rest/v1/rpc/<fn>` with the publishable key; codes are passed as args. `save_project` is durable only on `{ok:true}` and returns a `conflict` flag.
- **`supabase-client.js`** (IIFE → `window.SupabaseClient`): pure `buildRpcRequest`/`parseRpcResponse` + async `rpc()` + typed wrappers (`loadProject`, `saveProject`, `projectHistory`, `projectVersion`, `teacherClasses`, `createClass`, `addStudentsBulk`, `teacherRoster(code, classId)`, `appendStudent(code, classId, …)`, `reprintCodes(code, classId)`, `markComplete`).
- **`project-source.js`** (IIFE → `window.ProjectSource`, CommonJS for tests): pure starter-assembly + `meaningfulLineCount` / `starterMeaningfulLines(def)`, mirroring `project.js`. The dashboard uses it to compute the same net-lines baseline the student History shows.
- **`cover-sheet.js`** (`window.CoverSheet`, jsPDF + qrcode): `generateFinalSheets(...)` (progress pack) and `generateCodeTable({className, school, rows, scale})` (Name+Code grid; `scale: 2` = large cut-up strips).
...
### Notes for the remaining work (see the triage list above)
- **Start cover sheet** (only cover-sheet piece left): a second one-page-per-student PDF with assessed-prose front matter + a milestone checklist. Reuse the identity block from `generateFinalSheets`; the QR → mobile game view and short typeable URL already exist.
- **Scheduled jobs (P2)**: `pg_cron` for 12-month name auto-deletion (§5) and post-completion snapshot compaction (§6).
- **Dev/prod split + CI (P2)**: §13.7.
