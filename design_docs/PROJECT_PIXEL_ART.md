...
| 3 | Persistence between events | **Plain global variables** in an editable Setup section, named `player_x = 10`, `player_y = 10`. Students write `global player_x` followed by `player_x -= 1` in handlers — the `global` line is shown as part of the starter body and explained in the left-handler guidance ("the `player_x` I mean is the one from Setup"). Earlier iterations: a `state` dict (rejected — dictionaries weren't taught), plain `x`/`y` (rejected — `for x in range(20)` in `draw_border` clobbered the global), and auto-injected `global` (rejected — hidden injected lines broke the goal of "editor line numbers match what Pyodide reports in errors"). The `player_` prefix prevents loop-variable collisions; visible `global` keeps the assembled program lined up with what students see. The locked `use_canvas(GRID20)` runs as a JS-driven step before student code, so it doesn't take a line number. |
